 ...
 *
 *  lib for -Display with ssd1306/sh1106-Controller
...
{
    if (x > (DISPLAY_WIDTH / sizeof(FONT[0])) || y > (DISPLAY_HEIGHT / 8 - 1))
...
void lcd_putc(char c)
{
    ...
    default:
        ...
        if ((cursorPosition.x >= DISPLAY_WIDTH - sizeof(FONT[0])) || (c < ' '))
            break;
        // mapping char
        c -= ' ';
        ...
        * /
        ...
        else
        {
            ...
            {
                // print font to ram, print 6 columns
                // i2c_byte(1);//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                //i2c_byte(pgm_read_byte(&(FONT[(uint8_t)c][i])));
                i2c_write(pgm_read_byte(&(FONT[17][i])));
            }
            i2c_stop();
            cursorPosition.x += sizeof(FONT[0]);
        }
